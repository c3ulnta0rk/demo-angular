...
export class C3OverlayService {
   ...
   */
  public open<T>(component: Type<T>, config?: OverlayConfig<T>): C3OverlayRef<T> {
    // 1) Injecter un OverlayPane dans l’overlay container
    const overlayPaneRef = this._injector.injectComponent(C3OverlayPaneComponent);

    if (!overlayPaneRef) throw new Error('Could not create overlay pane');

    // On peut configurer le backdrop, la position, etc.
    if(config) {
      overlayPaneRef.setInput('hasBackdrop', config.hasBackdrop !== undefined ? config.hasBackdrop : true);
      overlayPaneRef.setInput('backdropClass', config.backdropClass || '');
      overlayPaneRef.setInput('position', config.position || 'center');
      overlayPaneRef.setInput('closeOnOutsideClick', config.closeOnOutsideClick !== undefined ? config.closeOnOutsideClick : true);
      overlayPaneRef.setInput('disposeOnNavigation', config.disposeOnNavigation !== undefined ? config.disposeOnNavigation : true);
    }
    ...
    paneElement.contentReady.subscribe(() => {
      ...
      const componentRef = this._injector.injectComponent(component, paneElement.contentRef().nativeElement);
  ...
  public closeAll(): void {
    for (const overlayRef of this._overlayRefs)
      overlayRef.close();
  }
}
